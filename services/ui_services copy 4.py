...
class ModernUIApp:
    def __init__(self, root):
        ...
        # Add an entry widget
        self.entrycoach = ttk.Entry(self.main_frame)
        self.entrycoach.grid(row=3, column=0, pady=5, sticky="ew")
        self.entrycoach.bind("<KeyRelease>", self.on_entry_change)
        ToolTip(self.entrycoach, "Enter Coach nos.")
    ...
    def process_data(self):
        try:
            ...
            # cls=OPCUAClient()
            # cls.Test_Bit_C1_Sen()
            # print("iterations from TestTrain.py",cls.itr)
            #######################################################################
            value=1
            while True:
                value +=1
