...
class ModernUIApp:
    def __init__(self, root):
        ...
        print("hhhhhhhhhhhhhhhhhhhhhhhhhhhhhh",self.all_data)
        sort=self.all_data[1]
        print(sort,"counttttttttttttttttttttttttttttttttttttttttttttttt")
        print("Total count of elements:", len(self.all_data))
        elements=len(self.all_data)
        for elements in self.all_data:
            print("all elements",elements)
        # Check if all_data is not None before iterating
        for i in range(1, 6):
            
            
            
            print(self.all_data,"all_data")
            
            print("dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd",self.all_data,"dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd")
            tikname = "LHB" if i == 1 else "Tik"
            train_type=tikname
            t=self.fetch_query(train_type)
            print("t",t)
            ttk.Radiobutton(self.main_frame, text=f"{tikname} {i}", variable=self.tik_var, value=str(i)).grid(row=5+i, column=0, sticky="w")
        ...
        self.button.grid(row=17, column=0, pady=20)
        ...
        self.theme_button.grid(row=21, column=0, pady=10)
    ...
    def insert_query(self,values_placeholder):
                ...
                try:
                    ...
                    if db.check_connection():
                        ...
                        insert_query = """
                        INSERT INTO [dbo].[test_train_userinput] ([speed_km/h])
                        VALUES (?)
                        """
    ...
    def process_data(self):
        try:
            ...
            tik_dropdown_value = self.tik_dropdown.get()
            ...
            output_text = (
                f"Hello, {user_input}!\n"
                f"Tik (Dropdown): {tik_dropdown_value}\n"
                f"Tik (Radio): {tik_radio_value}\n"
                f"Tik (Checkboxes): {', '.join(tik_check_values)}"
            )
    ...
    def load_preferences(self):
        try:
            with open("preferences.json", "r") as f:
                ...
                self.tik_dropdown.set(preferences["tik_dropdown"])
