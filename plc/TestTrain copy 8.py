...
class OPCUAClient:
    def __init__(self):
        ...
        self.event = threading.Event()  # Event to signal Loop 2
    ...
    def ExCounter_For_C1_Sen(self, polling_interval=0.00001, timeout=15):
        """
    ...
    def Test_Bit_C1_Sen(self, polling_interval=2):
        ...
        try:
            ...
            while iterations < max_iterations and not self.loop1_done:
                self.event.set()  # Signal Loop 2 to run
            ...
            self.event.set()  # Ensure Loop 2 exits after last run
        except Exception as e:
            print(f"Error writing to the node: {e}")
    ...
    def Test_Bit_S_Sen(self, polling_interval=1):
        ...
        try:          
            ...
            while iterations < max_iterations:
                self.event.wait()  # Wait for signal from Loop 1

                if self.loop1_done:  # Break if Loop 1 is finished
                    break
                # Increment the iteration counter at the start of each iteration
                iterations += 1
                ...
                # time.sleep(2)
                #time.sleep(self.time_seconds)
                self.event.clear()  # Reset event to wait for the next signal
        except Exception as e:
            print(f"Error writing to the node: {e}")
    """
